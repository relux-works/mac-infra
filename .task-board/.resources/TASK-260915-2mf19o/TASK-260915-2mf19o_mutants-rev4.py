#!/usr/bin/env python3
"""Narrowing-mutant harness for TASK-260915-2mf19o rev4.
...
NEW_MUTANTS = [
 ...
 dict(id='M28b', finding='F11', file=MAIN, name='meta set --json-value converts the parsed json.Number to float64 (rev3 shape)',
      ...
      old='\t\t\tif err := decodeSingleJSON([]byte(positional[2]), &value); err != nil {\n\t\t\t\treturn out.fail(usagef("--json-value: %v", err))\n\t\t\t}\n',
      new='\t\t\tif err := decodeSingleJSON([]byte(positional[2]), &value); err != nil {\n\t\t\t\treturn out.fail(usagef("--json-value: %v", err))\n\t\t\t}\n\t\t\tif number, ok := value.(json.Number); ok {\n\t\t\t\tvalue, _ = number.Float64()\n\t\t\t}\n',
