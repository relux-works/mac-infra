...
### 2010 — mac-keyvault T2 rev2: an embedded interface method is a guard bypass, and a bytes-only policy check belongs before the backend boundary
- FINDING (review rev1 F1): `GuardedStore` embeds `Backend` and overrode `Create`/`Delete`/`UpdateTag` only; Go promoted `Backend.Sign` unchanged, so the integration guard that keeps tests inside `works.relux.mac-keyvault.key.test.*` did not cover the private-key operation added in T2 — the rev1 results artifact claimed it did. Fix: `(*GuardedStore).Sign` calls `guard` first; `TestGuardedStoreRefusesNonTestLabels` now drives `Sign` with a signing-capable production label (a leak would return a real signature, not `ErrNotFound`) and a test-label positive control. Rule recorded in the type comment: every mutating or private-key-using `Backend` method must be overridden on the guard; embedding is where bypasses come from.
- FINDING (review rev1 F2): `runVerify` refused high-S after `Manager.PublicKeyFor` → `Backend.List`, so a malleable signature by label caused a Security call before the policy refusal, against the T1 "no backend call before validation" guard. The check only needs the parsed `(r, s)`, so it now sits directly after `ParseSignature`, before the `--spki` file read and before the address is resolved; the refusal envelope carries the digest/format/low_s inputs but no key fields. `TestRunVerifyHighSRefusedBeforeStore` prepares signatures with `crypto/ecdsa` (never via `run(sign)`), uses a fresh recording store holding the key, and asserts zero calls plus two ordering witnesses (absent address and absent `--spki` path are still `high_s_refused`, not `not_found`/usage). Mutants M16 (rev1 order), M17 (early gate DER-only), M18 (early gate `--spki`-only) are all killed; M4 from rev1 only proved the raw refusal existed.
- SCOPE: `TASK-260915-nl5may` rev2, files `internal/keyvault/security_darwin_test.go`, `cmd/mac-keyvault/main.go` (`runVerify`), `cmd/mac-keyvault/sign_test.go`, README, SKILL.

### 1930 — mac-keyvault T2: sign/verify go through the same trust gate as rotate, and the low-S rule is applied on the way out, not assumed from the bridge
- FINDING: `SecKeyCreateSignature(kSecKeyAlgorithmECDSASignatureDigestX962SHA256)` signs the given 32 bytes as the digest (no re-hash: the DER verifies with `crypto/ecdsa` and `openssl dgst -sha256 -verify` over the original payload) and returns X9.62 DER that Go's `encoding/asn1` parses strictly; whether corecrypto folds s into the low half is not documented, so the vault does not assume it: every signature is parsed, folded to low-S (`normalized: true` when it was high), and re-verified under the record's own SPKI before it is emitted. The same non-assumption holds for `crypto/ecdsa`, which demonstrably emits high-S about half the time; the test fakes therefore pick the half deterministically instead of trusting the library.
- DECISION: The operations gate is derived from the registry (`Operations()`), not re-implemented: `Manager.authorize` requires the operation to be in what `describe` prints and names the reason of a miss by its own code (`usage_refused`, `expired`, `unsupported_primitive` exit 1). Before that it runs the rev6/rev7 trust gate (`RecordProblem`, schema, `ValidateStored`), so a rev1 tag or a forged persisted field never reaches the private key (`metadata_unknown`) — the forgery table in `invariants_test.go` now drives `Sign` and `PublicKeyFor` as well as rotate/meta.
- DECISION: `verify` has three outcomes with three exit codes: 0 verified; 1 `signature_invalid` (well-formed, does not match — the verdict is in `result.verified`, the error contract still carries code/hint); 3 `invalid_signature` (not strict DER / not 64 raw bytes / r,s outside [1,n-1]) or `high_s_refused` (valid but malleable; `--allow-high-s` opts in, needed for openssl-made signatures). A `@FILE` argument is always raw bytes, never hex-decoded, so a 32-byte digest that happens to look like ASCII hex is not misread (mutant M10).
- FINDING: Two mutants survive and state bounds, not gaps: M3 (strict re-encode check removed) survives because `encoding/asn1` already refuses non-minimal integers and long-form lengths one layer down — the re-encode check is defence in depth; M12 (CLI digest-length check removed) survives because `Manager.Sign` validates the digest before listing — the combined M12c (both gates after the listing) is killed by the CLI store-untouched assertion.
- SCOPE: `TASK-260915-nl5may`: `internal/keyvault/ecdsa.go` (new), `keyvault.go` (`Backend.Sign`, `authorize`, `Sign`, `PublicKeyFor`), `registry.go` (via sign/verify), `security_darwin.{c,h,go}` + `security_unsupported.go` (bridge), `cmd/mac-keyvault/main.go` (`sign`, `verify`), tests, README, SKILL. kvctl's verifier is not in this repository; interop is proven against Go `crypto/ecdsa` and openssl 3.6 instead (stated bound).

### 1845 — mac-keyvault rev7: a validator can only judge what the decoder kept — the persisted record decoder is strict about unknown members
- FINDING: Review rev6 (F14) reproduced the F4/F8/F12/F13 class one layer earlier: `DecodeSingleJSON` decoded a schema-2 tag without `DisallowUnknownFields`, so a persisted member the schema does not define (`future_top_level_policy`, `format.future_format_policy`) was erased by `encoding/json` before `ValidateStored` ran; `Manager.Rotate` reproduced the record minus the member (`creates=1`) and `MetaSet` rewrote the tag without it. The one-row-per-field invariant table reflects over the decoded Go struct and is blind to JSON members that never reach the struct — the reviewer's control named exactly that blind spot.
- DECISION: The strictness lives in the decoder, not in another table row. `DecodeSingleJSON` now sets `DisallowUnknownFields` next to `UseNumber` and the EOF gate, so the one JSON boundary rule (rev5) covers members as well as documents; unknown top-level and nested struct members make the tag an unreadable record (schema 0, `record_unreadable` finding, `rotate`/`meta set|unset` refuse with `metadata_unknown`, zero creates/updates, old tag byte-identical). `Record.Meta`, `--meta-json` (`map[string]any`) and `--json-value` (`any`) are open targets and unaffected — the positive controls store the same member names inside `meta` and still rotate. A field that starts driving behaviour is added to the struct with a schema bump, as model v2 §2–§3 say; until then it cannot be silently downgraded.
- FINDING: Narrowing mutant M31 (strict rejection removed, EOF gate and `ValidateStored` intact) is killed by `TestRunUnknownRecordFieldIsUnreadableEverywhere` (4/4 shapes through `run(...)`) and by `TestRecordEncodingAndLegacyTags`; with the fix present both are green and no other test changed behaviour.
- SCOPE: `TASK-260915-2mf19o` rev7 (CR rev6 F14 rework, orchestrator directive "nothing else"): `internal/keyvault/record.go` (one line + doc), `internal/keyvault/keyvault_test.go`, `cmd/mac-keyvault/main_test.go`, README, SKILL, this entry.

### 1815 — mac-keyvault rev6: forged persisted fields are a class, not a field — one per-kind invariant table for every record field, run by every write and reported by every read
- FINDING: Reviews rev2-rev5 found four forged-persisted-field bypasses one at a time (issuer F8, trailing document F12, `validity.not_before` F13 — a key with `{not_before: T, not_after: null}` passed `ValidateNew`, whose only validity check was ordering when both bounds were present, and `Manager.Rotate` reproduced it: reviewer probe `creates=1`). Per-field patches recur because each new field gets its own ad-hoc `if`. The class is now closed structurally: `internal/keyvault/invariants.go` holds one table with exactly one row per JSON field of `Record` in declaration order (schema=2, kind vocabulary, service/purpose charset, version ≥ 1, title ≤ 80 code points, description free, algorithm per kind, store, extraction per kind + enclave never extractable, usages non-empty subset without duplicates, format keys and vocabulary per kind, created non-null, origin user/host/tool present and source `generated` | `imported:<sha256 hex>`, issuer null unless certificate, validity not_before null unless certificate and certificate needs both, meta reserved names/types); `user_presence` is the one stated bound (request flag the backend answers). `TestRecordInvariantsCoverEveryField` compares the row list with the struct's JSON tags, so a field added without a row fails at compile-adjacent time.
- FINDING: One validator, four call sites, two behaviours: `ValidateNew` (init) runs the table; `ValidateStored(label, rec)` adds label agreement and is run by `Rotate` and by `updateMeta` (meta set/unset — a meta write would otherwise re-persist a forged field as if valid) as a `metadata_unknown` refusal with zero creates/updates and the old tag byte-identical; `Key.Findings` runs it on every read and reports `record_invalid:<code>` alongside `unsupported_primitive`, never a refusal, so a forged item stays visible for `delete`. `created` is stamped before validation (init and rotate) so the validated record is the stored record; `localOrigin` records `unknown` instead of an empty user/host when the lookup fails.
- FINDING: The shared forgery table `internal/keyvault/recordtest` (28 forged shapes + label mismatch + 2 positive controls) drives both suites through `Manager.*` and `run(...)`; the rev6 mutants (M30-M30h) weaken exactly one row or one call site each and every one is killed by a named test. Also: the F2 early-lock-release mutant (M2) had been killed by scheduler luck; the fake store now gates `Create` until both callers listed, so the kill is deterministic (3/3).
- ANOMALY: `TestSecurityStoreKeychainRoundTrip` (real login keychain) failed once in four package runs with "created key missing from list" while `cmd/mac-keyvault` ran against the same keychain in parallel; 3/3 green afterwards and green in the attached full run. Not in this delta's scope; recorded as a pre-existing cross-process listing flake to watch.
- SCOPE: `TASK-260915-2mf19o` rev6 (CR rev5 F13 rework, class of F8/F12; orchestrator directive): `internal/keyvault/{invariants.go,record.go,keyvault.go,recordtest/}`, `cmd/mac-keyvault/main.go`, tests, README, SKILL.
...
### 1720 — mac-keyvault rev5: the JSON document-boundary rule has to be one implementation for every JSON the vault reads, persisted records included
- FINDING: The F6 document-boundary class recurred through the persisted-record decoder: `--meta-json` and `--json-value` required `io.EOF` after the first value (`decodeSingleJSON`), but `DecodeRecord` decoded `kSecAttrApplicationTag` with a bare `json.Decoder.Decode`, so `{record} {record}` was reported as a readable schema-2 record and `Manager.Rotate` created the next generation from it (reviewer probe: `creates=1`). Two boundaries with two decoders drift; the EOF-required decoder is now `keyvault.DecodeSingleJSON`, called by `--meta-json`, `meta set --json-value` and `DecodeRecord`, so a second document, token or garbage after a persisted record is a `RecordProblem` (rotate: `metadata_unknown`, zero creates, old tag untouched) and only whitespace around one document stays readable.
- FINDING: A narrowing mutant on the shared decoder now has to be killed at both boundaries (M23/M23b name the `--meta-json` and the rotate regressions); M29 admits exactly the `{record} {record}` tag shape in `DecodeRecord` alone (garbage and tokens still refused, input decoder untouched) and M29b restores the rev4 bare decoder — both killed by `TestManagerRotateRefusesUnknownMetadata/trailing_*`, `TestRunRotateRefusesUnknownMetadata/trailing_*` and `TestRecordEncodingAndLegacyTags`.
- SCOPE: `TASK-260915-2mf19o` rev5 (CR rev4 F12 rework, repeat-of F6): `internal/keyvault/record.go`, `cmd/mac-keyvault/main.go`, tests.
...
### 1640 — mac-keyvault rev4: mode detection by string matching was the F3 root cause; closed vocabularies must guard every flag; numeric meta needs json.Number end to end
- FINDING: The usage-error envelope contract (F3 → F6 → F9) recurred three times because `--json` was detected by string comparison against four spellings while the commands parse it with `flag.Bool`, which accepts `1/t/T/TRUE/true/True`; `version --json=TRUE extra` was routed to text mode. The detector is now the same parser: each token is offered alone to a `flag.FlagSet` that defines only `json`, so the accepted set is identical by construction, a later `--json=false` wins like a repeated flag, and `--` ends detection and command parsing alike (`parseFlags` no longer re-parses tokens after a consumed `--`). Stated bound: a `--json` token consumed as the value of a string flag (`--out --json`) belongs to the command parser.
- FINDING: `list --kind bogus` reached `Backend.List` and answered `[]` because only `--service` was validated on the list path. `ValidateKind` is one closed-vocabulary gate shared by `ParseAddress` and `list`, applied before the manager exists.
- FINDING: Numeric meta lost precision twice: `meta set --json-value` converted `json.Number` to `float64`, and `DecodeRecord`/`keyView` decoded without `UseNumber`, so `9007199254740993` was persisted, printed and rotated as `…992`. Numbers are `json.Number` from input to tag to envelope; 2^53+1 and 2^64+1 round-trip through init, describe, list, meta set/get and rotate digit-exact.
- SCOPE: `TASK-260915-2mf19o` rev4 (CR rev3 F9-F11 rework, repeat-of F6/F3): `cmd/mac-keyvault`, `internal/keyvault/record.go`, README.

### 1513 — mac-keyvault rev3: usage gate must be per-command, JSON input must be a single document, bounds are code points, kind invariants re-checked on rotate
- FINDING: The rev2 usage-error contract (F3) was bypassable in two spots the common emitter never saw: `version` was dispatched without parsing its arguments (`--json version extra` returned a success envelope, exit 0), and `--meta-json` decoded one JSON value without requiring EOF, so `{"owner":"a"} {"ignored":true}` created a key with only the first object. A shared emitter proves nothing about a command that does not call the parser; every command now goes through `parseFlags` and `requireNoArguments`, and all JSON input (`--meta-json`, `meta set --json-value`) goes through `decodeSingleJSON`, which requires `io.EOF` after the first value.
- FINDING: `title ≤ 80 chars` was enforced with `len()` (bytes), refusing an 80-letter Armenian title as "160 characters". Bounds on human text count code points (`utf8.RuneCountInString`).
- FINDING: `ValidateNew` never checked `issuer`, so a stored schema-2 key record with a forged non-null issuer rotated into the next generation. `validateIssuer` enforces the kind invariant (certificate: complete `{dn, fingerprint}`; every other kind: null) on init and, through `ValidateNew(next)`, on rotate (`metadata_unknown`, zero creates).
- FINDING: `TestManagerInitDuplicateRefusalIsAtomic`'s no-lock positive control was scheduler-dependent: under `go test ./...` load the first goroutine could list, return and create before the second listed, so the "race" produced one create. The fake store now holds each snapshot on a second barrier until the peer has taken its own; 60 stress runs across `-cpu 1,2,8` green, M2 (lock released before create) still killed.
- SCOPE: `TASK-260915-2mf19o` rev3 (CR rev2 F6-F8 rework): `cmd/mac-keyvault`, `internal/keyvault`, README.

### 1425 — mac-keyvault rev1 Keys Were Extractable; Legacy SecKey Attributes Only Count At Top Level
- FINDING: `kSecAttrIsExtractable=false` and `kSecAttrAccess` placed inside `kSecPrivateKeyAttrs` are silently ignored by the legacy (login keychain) path of `SecKeyCreateRandomKey`: rev1 keys had `CSSM_KEYATTR_EXTRACTABLE` set and `SecKeyCopyExternalRepresentation`/`SecItemExport` returned the private key (321 bytes); a two-application trusted list inside the private dict produced a one-entry creator-only ACL. At the top level both are honoured: export fails with `-25316 errSecDataNotAvailable`, the ACL lists the given applications verbatim, and the public half stays readable. Review finding F5 was exact; `TestSecurityStorePrivateKeyIsNotExtractable` and `TestSecurityStoreACLTrustsOnlyCallingBinary` now attest both properties and narrowing mutants (rev1 shape, second app, `SecACLSetContents(NULL)`) are killed.
- FINDING: `SecItemUpdate` with `kSecAttrApplicationTag` on a legacy key writes the value into the PrintName (label) attribute and leaves the tag untouched, orphaning the item; three probe keys had to be deleted by exact (label, tag) pair. `SecKeychainItemModifyAttributesAndData(kSecKeyApplicationTag)` updates the record in place without a prompt.
- FINDING: `SecAccessCreate(NULL)` yields a creator-only ACL, so a "trust everyone" mutant must rewrite the ACL entries; a NULL list is not a broadening mutant.
- DECISION: Consolidated record model v2 lands in `kSecAttrApplicationTag` as schema-2 JSON under `works.relux.mac-keyvault.<kind>.<service>.<purpose>.v<N>` (uniqueness `(kind, service, purpose, version)`, raw labels refused, `origin` replaces `issuer` for keys); v1 tags read as schema 1 with service/purpose/kind/algorithm unknown and are never upgraded; `rotate`/`meta set` refuse them (`metadata_unknown`). `--extraction human|agent` is validated and persisted as policy (the ACL mechanics are T7's; every key stays non-extractable at the keychain level in this revision), and only kind `key` is created. Errors follow the §8 contract `{code, message, hint, os_status}`; -34018 is `missing_entitlement` (exit 1).
- DECISION: Duplicate refusal is made atomic with an exclusive `flock` (`~/Library/Application Support/mac-infra/keyvault/init.lock`) held across the single listing and the create; the legacy keychain accepts duplicate labels itself.
- SCOPE: `TASK-260915-2mf19o` rev2 (F1-F5 rework + model v2): `cmd/mac-keyvault`, `internal/keyvault`, README, SKILL.

### 1342 — mac-keyvault Store Is Login Keychain + Legacy SecAccess
- FINDING: On an ad-hoc signed CLI, `kSecAttrAccessControl` (even plain `kSecAccessControlPrivateKeyUsage`) routes a permanent key into the Data Protection keychain and fails with -34018 exactly like the Secure Enclave and `kSecAccessControlUserPresence` shapes; only `kSecAttrAccess` from `SecAccessCreate` with a `SecTrustedApplicationCreateFromPath(NULL)` list persists in the login keychain.
- FINDING: The legacy key schema returns no creation date, so `internal/keyvault` stores `store=`/`created=` in `kSecAttrApplicationTag` and reports `unknown` when the tag or public half is unreadable.
- DECISION: `internal/keyvault` is a C cgo bridge (no Swift helper); `--enclave`/`--user-presence` make the real Security call and refuse with the OSStatus; no keychain fallback.
- ANOMALY: Raw public-key bytes crossing the bridge inside a byte-delimited record broke `list` as soon as a generated point contained `0x1e`; the bridge now hex-encodes the point and `TestParseListRecords` pins it.
- SCOPE: `TASK-260915-2mf19o` (`cmd/mac-keyvault`, `internal/keyvault`, setup/deinit, README, SKILL); no sign/verify yet.
